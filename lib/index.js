...
exports.init = function(startup_time, config, events) {
  ...
  var connection_queue = async.queue(function(task, callback) {
    if (dbs[task.name]) {
      ...
      const { host, port, name, authMechanism, authSource } = options;
      // const mongoUrl = `mongodb://${host}:${port}/${name}?authMechainsm=${authMechanism}&authSource=${authSource}`;
  ...
  options.authMechanism = config.mongoAuthMechanism || 'SCRAM-SHA-1';
  options.authSource = config.mongoAuthSource || options.name;
